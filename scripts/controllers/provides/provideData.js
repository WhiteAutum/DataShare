...
//    * 创建数据
//    */
//   $scope.provideData = function (address, password, dataName, introduction, capability,expireTime,beganTime,depth,
...
//       //添加数据源
//       //2019.4.11修改函数
//       contractInstance.MainCreateData(formatBytes(dataName), introduction,expireTime,beganTime,depth,formatBytes(capability),formatBytes(introduction),capability, formatBytes(cap_pwd), {
//         from: address,
//         gas: 80000000
//       });
//       //添加数据类型
...
angular.module("provideData", []).controller("provideData", function ($scope) {
   ...
   * 创建数据
   */
  $scope.provideData = function (address, password, dataName, introduction, capability, cap_pwd,ipfs,select_tool_type, select_purpose_type)
  {
    if (!address || !password || !dataName || !introduction || !capability || !cap_pwd || !select_tool_type || !select_purpose_type)
    {
      alert("请填写完善信息！");
      return;
    }
    ...
    //添加数据
    try {
      //添加数据源     createData(bytes32 daNa, string intro, string cap, bytes32 cap_pwd,bytes32 _dap)
      contractInstance.createData(formatBytes(dataName), introduction, capability, formatBytes(cap_pwd), formatBytes(ipfs), {
        from: address,
        gas: 80000000
      });
      //添加数据类型
